...
for i=1:n
  ...
  CPT{i} = convert_to_pot(bnet.CPD{bnet.equiv_class(i)}, fam(:), evidence);
end
...
for i=1:n
  b = bucket_num(CPT{i}.domain, order);
...
for i=sum_over(:)'
  ...
  rest = mysetdiff(B{j}.domain, i);
  % minka
  if ~isempty(rest)
    temp = marginalize_pot(B{j}, rest);
    b = bucket_num(temp.domain, order);
